...
void SignalCaster::registerCast(const std::type_info& type, SignalCaster::displayer_type displayer,
...
SignalCaster::cast_functions_type& SignalCaster::getCast(const std::string& type_name) {
  std::map<std::string, cast_functions_type>::iterator it = functions_.find(type_name);
...
std::vector<std::string> SignalCaster::listTypenames() const {
  ...
  for (std::map<std::string, cast_functions_type>::const_iterator iter = functions_.begin(); iter != functions_.end();
       iter++)
    typeList.push_back(iter->first);
  return typeList;
}

boost::any SignalCaster::cast(const std::type_info& type, std::istringstream& iss) {
