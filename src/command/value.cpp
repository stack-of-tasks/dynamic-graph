...
Value::Value(const unsigned& value) : type_(UNSIGNED), value_(new unsigned(value)) {}
...
Value::Value(const std::string& value) : type_(STRING), value_(new std::string(value)) {}
Value::Value(const Vector& value) : type_(VECTOR), value_(new Vector(value)) {}
Value::Value(const Eigen::MatrixXd& value) : type_(MATRIX), value_(new Eigen::MatrixXd(value)) {}
Value::Value(const Eigen::Matrix4d& value) : type_(MATRIX4D), value_(new Eigen::Matrix4d(value)) {}

Value::Value(const Value& value) : type_(value.type_), value_(copyValue(value)) {}
...
unsigned Value::unsignedValue() const {
  ...
  throw ExceptionAbstract(ExceptionAbstract::TOOLS, "value is not an unsigned int");
}
...
float Value::floatValue() const {
  float result;
  if (FLOAT != type_) throw ExceptionAbstract(ExceptionAbstract::TOOLS, "value is not a float");
...
double Value::doubleValue() const {
  double result;
  if (DOUBLE != type_) throw ExceptionAbstract(ExceptionAbstract::TOOLS, "value is not a double");
...
Eigen::MatrixXd Value::matrixXdValue() const {
  ...
  throw ExceptionAbstract(ExceptionAbstract::TOOLS, "value is not a Eigen matrixXd");
}
