...
void Entity::entityRegistration() { PoolStorage::getInstance()->registerEntity(name, this); }

void Entity::entityDeregistration() { PoolStorage::getInstance()->deregisterEntity(name); }
...
Entity::~Entity() {
  ...
  for (std::map<const std::string, Command*>::iterator it = commandMap.begin(); it != commandMap.end(); it++) {
    delete it->second;
  }
  dgDEBUGOUT(25);
}
    ...
    if (sigkey != signalMap.end())  // key does exist
    {
      ...
      if (sigkey->second != &sig) {
        throw ExceptionFactory(ExceptionFactory::SIGNAL_CONFLICT,
                               "Another signal already defined with the same name. ", "Signame is <%s>.",
                               signame.c_str());
      }
    } else {
      dgDEBUG(10) << "Register signal <" << signame << "> for entity <" << getName() << "> ." << endl;
...
void Entity::signalDeregistration(const std::string& signame) {
  ...
  if (sigkey == signalMap.end())  // key does not exist
  {
    ...
    throw ExceptionFactory(ExceptionFactory::UNREFERED_SIGNAL, "No signal defined with the given name. ",
                           " (while erasing <%s>).", signame.c_str());
  } else {
    dgDEBUG(10) << "Deregister signal <" << signame << "> for entity <" << getName() << "> ." << endl;
...
#define __DG_ENTITY_GET_SIGNAL__(ITER_TYPE)                                                                      \
  ...
  if (sigkey == signalMap.end()) /* key does NOT exist */                                                        \
  {                                                                                                              \
    throw ExceptionFactory(ExceptionFactory::UNREFERED_SIGNAL, "The requested signal is not registered", ": %s", \
                           signame.c_str());                                                                     \
  }                                                                                                              \
  return *(sigkey->second);

bool Entity::hasSignal(const string& signame) const { return (!(signalMap.find(signame) == signalMap.end())); }

SignalBase<int>& Entity::getSignal(const string& signame) { __DG_ENTITY_GET_SIGNAL__(iterator); }

const SignalBase<int>& Entity::getSignal(const string& signame) const { __DG_ENTITY_GET_SIGNAL__(const_iterator); }
...
void Entity::display(std::ostream& os) const { os << this->getClassName() << ": " << name; }
...
const std::string& Entity::getCommandList() const { return Entity_COMMAND_LIST; }
...
std::map<const std::string, Command*> Entity::getNewStyleCommandMap() { return commandMap; }
...
void Entity::sendMsg(const std::string& msg, MsgType t, const char* file, int line) {
