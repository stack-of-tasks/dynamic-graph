...
#define __TIME_DEPENDENCY_INIT(sig, dep)                                                       \
  leader(*sig), dependencies(), updateFromAllChildren(ALL_READY_DEFAULT), dependencyType(dep), \
      periodTime(PERIOD_TIME_DEFAULT)

namespace dynamicgraph {
template <class Time>
TimeDependency<Time>::TimeDependency(SignalBase<Time>* sig, const DependencyType dep)
    : __TIME_DEPENDENCY_INIT(sig, dep) {}

template <class Time>
TimeDependency<Time>::TimeDependency(SignalBase<Time>* sig, const SignalArray_const<Time>& ar,
...
bool TimeDependency<Time>::needUpdate(const Time& t1) const {
  dgTDEBUG(15) << "# In {" << leader.getName() << " : (" << leader.getReady() << ") " << t1 << " >? "
...
template <class Time>
std::ostream& TimeDependency<Time>::displayDependencies(std::ostream& os, const int depth, std::string space,
