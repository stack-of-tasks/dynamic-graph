...
class SignalBase : public boost::noncopyable {
 public:
  explicit SignalBase(std::string name = "") : name(name), signalTime(0), ready(false) {}
  ...
  void getClassName(std::string& aClassName) const { aClassName = typeid(this).name(); }
  ...
  virtual std::ostream& displayDependencies(std::ostream& os, const int = -1, std::string space = "",
  ...
  virtual void plug(SignalBase<Time>* sigarg) {
    DG_THROW ExceptionSignal(ExceptionSignal::PLUG_IMPOSSIBLE, "Plug-in operation not possible with this signal. ",
                             "(while trying to plug %s on %s).", sigarg->getName().c_str(), this->getName().c_str());
  }
  ...
  virtual void set(std::istringstream&) {
    DG_THROW ExceptionSignal(ExceptionSignal::SET_IMPOSSIBLE, "Set operation not possible with this signal. ",
                             "(while trying to set %s).", this->getName().c_str());
  }

  virtual void get(std::ostream&) const {
    DG_THROW ExceptionSignal(ExceptionSignal::SET_IMPOSSIBLE, "Get operation not possible with this signal. ",
                             "(while trying to get %s).", this->getName().c_str());
  }

  virtual inline void recompute(const Time&) {
    DG_THROW ExceptionSignal(ExceptionSignal::SET_IMPOSSIBLE, "Recompute operation not possible with this signal. ",
                             "(while trying to recompute %s).", this->getName().c_str());
  }

  virtual void trace(std::ostream&) const {
    DG_THROW ExceptionSignal(ExceptionSignal::SET_IMPOSSIBLE, "Trace operation not possible with this signal. ",
  ...
  virtual void checkCompatibility() {
    DG_THROW ExceptionSignal(ExceptionSignal::PLUG_IMPOSSIBLE, "Abstract signal not compatible with anything.",
                             "(while trying to plug <%s>).", this->getName().c_str());
  }
  /// \}
