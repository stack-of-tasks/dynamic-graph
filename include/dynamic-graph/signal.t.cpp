...
#define __SIGNAL_INIT(name, Tcpy, Tref, TrefNC, mutex)                                                  \
  SignalBase<Time>(name), signalType(SIGNAL_TYPE_DEFAULT), Tcopy1(Tcpy), Tcopy2(Tcpy), Tcopy(&Tcopy1),  \
      Treference(Tref), TreferenceNonConst(TrefNC), Tfunction(), keepReference(KEEP_REFERENCE_DEFAULT), \
      providerMutex(mutex)

namespace dynamicgraph {

template <class T, class Time>
Signal<T, Time>::Signal(std::string name) : __SIGNAL_INIT(name, T(), NULL, NULL, NULL) {
  return;
}
...
void Signal<T, Time>::trace(std::ostream& os) const {
  try {
    ...
    DG_THROW ExceptionSignal(ExceptionSignal::SET_IMPOSSIBLE, "TRACE operation not possible with this signal. ",
                             "(bad cast while getting value from %s).", SignalBase<Time>::getName().c_str());
  }
}
...
template <class T, class Time>
void Signal<T, Time>::setFunction(boost::function2<T&, T&, Time> t, Mutex* mutexref) {
  signalType = FUNCTION;
  Tfunction = t;
  providerMutex = mutexref;
  copyInit = false;
